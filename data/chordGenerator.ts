...
import type { ChordInversions, EnharmonicMap, ChordGenerator } from '../types';

const noteOrder: string[] = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];

// Map of enharmonic equivalents
const enharmonicMap: EnharmonicMap = {
  'C#': 'Db',
  'D#': 'Eb',
  'F#': 'Gb',
  'G#': 'Ab',
 ...
 */
const getNoteAtInterval = (root: string, semitones: number): string | null => {
 ...
 */
const generateMajorChord: ChordGenerator = (root: string): ChordInversions => {
 ...
 */
const generateMinorChord: ChordGenerator = (root: string): ChordInversions => {
  const rootNote = root + '4';
  return {
    root: [
      rootNote,
 ...
 */
const generateDom7Chord: ChordGenerator = (root: string): ChordInversions => {
  const rootNote = root + '4';
  return {
    root: [
      rootNote,
 ...
 */
const generateMaj7Chord: ChordGenerator = (root: string): ChordInversions => {
  const rootNote = root + '4';
  return {
    root: [
      rootNote,
 ...
 */
const generateDimChord: ChordGenerator = (root: string): ChordInversions => {
  const rootNote = root + '4';
  return {
    root: [
      rootNote,
 ...
 */
const generateMin7Chord: ChordGenerator = (root: string): ChordInversions => {
  const rootNote = root + '4';
  return {
    root: [
      rootNote,
 ...
 */
const generateDim7Chord: ChordGenerator = (root: string): ChordInversions => {
  const rootNote = root + '4';
  return {
    root: [
      rootNote,
 ...
 */
const generateMin7b5Chord: ChordGenerator = (root: string): ChordInversions => {
  const rootNote = root + '4';
  return {
    root: [
      rootNote,
...
export const generateAllChords = () => {
  const allChords: { name: string; notes: ChordInversions }[] = [];
  
  // Helper function to add chord with enharmonic equivalent
  const addChord = (name: string, notes: ChordInversions) => {
